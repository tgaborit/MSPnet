...
int button_pressed = 0;                 // boolean value indicating a pressed button
...
int main( void )
{
  ...
  while(1){
    ...
    if(timer0_interrupt == 1)
    {
      ...
      // TODO
      // update output peripherals state and check if input was triggered
    }

    // check if timer 1 did an interrupt
    if(timer1_interrupt == 1){  
      // check if button is trully pressed
      if(~P1IN & 0x08)
      {
        button_pressed = 1;            // update button pressed flag to 1
      }
      
      P1IFG &=~0x08;                    // P1.3 IFG cleared
      P1IE = 0x08;                      // P1.3 only IRQ enable (reset button after debouncing mode)
...
// interrupt function when button goes from HIGH to LOW
#pragma vector=PORT1_VECTOR
__interrupt void Port_1(void)
{
  // starts debounce timer waiting time in order to check and validate pressed button
  if (P1IFG & 0x08)
  {
...
// interrupt function when 20ms wait cycle is ended to check and validate pressed button
